wpda_gallery_tree={
	...
	create_content:function(){	
		...
		jQuery('.wpda_gall_tree_content_parent').append('<div class="wpda_gall_load_trash1" title="Recycle Bin"></div><div id="wpda_gall_tree_content" class="wpda_gall_tree_content"></div>');
	...
	conect_add_galery_to_form:function(){	
		...
		jQuery('.wpda_gall_create_gallery').click(function(event){	                                           								//add event 27-03-2018
	...
	create_gallery_name_form:function(element){
		...
		// for security
		wp_nonce_securyty();
		jQuery('.wpda_gall_tree_gallery_form_name_conteiner').click(function(event){event.stopPropagation();});                                        //add event 27-03-2018
		jQuery('.wpda_gall_tree_gallery_form_cencel_button').click(function(event){jQuery(this).parent().remove();event.stopPropagation();});          //add event 27-03-2018
		...
		jQuery(element).find("button").eq(0).click(function(event){
			...
			if(form_create_gallery_name != ''&& gallery_names.indexOf(form_create_gallery_name.toLowerCase()) < 0){           //30-03-2018
	...
	conect_add_album_to_form:function(){	
		...
		jQuery('.wpda_gall_tree_create_album').click(function(event){                                          									//add event 27-03-2018
			//remove other opened forms
			self.tree_remove_inline_opened_boxes();
			// create album				
	...
	create_album_add_name_form:function(element){
		...
		jQuery('.wpda_gall_tree_album_form_name_conteiner').click(function(event){event.stopPropagation();});                                	//add event 27-03-2018
		jQuery('.wpda_gall_tree_album_form_cencel_button').click(function(event){jQuery(this).parent().remove();event.stopPropagation();});   	//add event 27-03-2018
		...
		jQuery(element).find("button").eq(0).click(function(event){
			...
			if(form_create_album_name != ''&& albom_names.indexOf(form_create_album_name.toLowerCase()) < 0){     //30-03-2018
				jQuery('.wpda_gall_load_trash').addClass('wpda_gall_display_inline');						
				jQuery.ajax({
	...
	conect_edit_gallery_to_tree:function(){	
		...
		jQuery('.wpda_gall_tree_galery_name').click(function(event){                                          								//add event 27-03-2018
	...
	create_edit_gallery_form_to_tree:function(element){
		...
		jQuery('.wpda_gall_edit_gallery_name_content').click(function(event){event.stopPropagation();});                                 		//add event 27-03-2018
		jQuery('.wpda_gall_tree_gallery_edit_form_cencel_button').click(function(event){self.remove_edit_gallery_form();return false;}); 		//add event 27-03-2018
		...
		jQuery(element).next().children().eq(0).children().eq(1).click(function(event){
			event.preventDefault();
			var form_gallery_index_name=jQuery("[name='wpda_gall_tree_gallery_form_name_input']").attr("value"); 
			var gallery_names = [];			
			for(x in wpda_gallery_tree.tree_info){
				gallery_names[x] = wpda_gallery_tree.tree_info[x].name.toLowerCase();						
			}						
			if(form_gallery_index_name != ''&& gallery_names.indexOf(form_gallery_index_name.toLowerCase()) < 0){                 //30-03-2018		
				jQuery('.wpda_gall_load_trash').addClass('wpda_gall_display_inline');					
				jQuery.ajax({
		...
		jQuery(element).next().children().eq(0).children().eq(2).click(function(event){                              							//add event 27-03-2018
			event.preventDefault();
	...
	conect_edit_album_to_tree:function(){	
		...
		jQuery('.wpda_gall_tree_album_name').click(function(event){                   												//add event 27-03-2018
			//remove other appened forms
			self.tree_remove_inline_opened_boxes();
			// create album				
	...
	create_edit_album_form_to_tree:function(element){
		...
		jQuery('.wpda_gall_edit_album_name_content').click(function(event){event.stopPropagation();});                                                 //add event 27-03-2018
		jQuery('.wpda_gall_tree_album_edit_form_cencel_button').click(function(event){self.remove_edit_album_form();return false;});                   //add event 27-03-2018
		...
		jQuery(element).next().children().eq(0).children().eq(1).click(function(event){
			event.preventDefault();		
			var form_album_index_name=jQuery("[name='wpda_gall_tree_album_form_name_input']").attr("value"); 
			var albom_names = [];			
			for(x in wpda_gallery_tree.tree_info[form_gallery_index]){
				albom_names[x] = wpda_gallery_tree.tree_info[form_gallery_index][x].name;
				albom_names[x] = albom_names[x]?albom_names[x].toLowerCase():albom_names[x];
			...
			if(form_album_index_name != ''&& albom_names.indexOf(form_album_index_name.toLowerCase()) < 0){	                      //30-03-2018			
				jQuery('.wpda_gall_load_trash').addClass('wpda_gall_display_inline');						
				jQuery.ajax({
	...
	drag_rows:function(event) {                                                             //28-03-2018
		event.dataTransfer.setData("Text", "");                                             //28-03-2018
	...
	drop_rows:function(event) {                                                            //28-03-2018
	...
	dragEnter:function(event) {//rechanged 09-09-2017
		if ( wpda_gallery_tree.drag_target != 'wpda_gall_drop_rows'){
			if ((wpda_gallery_tree.drag_target != 'wpda_gall_tree_open_close_album_right_span' && event.target.className == 'wpda_gall_tree_edit_img') || (wpda_gallery_tree.drag_target != 'wpda_gall_tree_open_close_album_right_span' && event.target.className == 'wpda_gall_tree_edit_img image_active') || event.target.className == 'wpda_gall_tree_open_close_album_right_span' || event.target.className == 'wpda_gall_tree_open_close_album_right_span image_active' || event.target.className ==  'wpda_gall_tree_open_close_gallery_right_span' || event.target.className ==  'wpda_gall_load_trash1') {    
	...
	dragLeave:function(event) {
		if (event.target.className == 'wpda_gall_tree_edit_img' || event.target.className == 'wpda_gall_tree_edit_img image_active' || event.target.className == 'wpda_gall_tree_open_close_album_right_span' || event.target.className == 'wpda_gall_tree_open_close_album_right_span image_active' || event.target.className ==  'wpda_gall_tree_open_close_gallery_right_span' || event.target.className ==  'wpda_gall_load_trash1') { 
	...
	drag:function(event) {
		//event.dataTransfer.setData("Text", event.target.id);                                             //28-03-2018
		event.dataTransfer.setData("Text", "");                                             //28-03-2018
	...
	drop:function(event) {
		event.preventDefault();               //01-04-2018
...
var wpda_gallery_edit_upload={
	...
	activete_image_tab:function(){
		...
		jQuery('#WpDevArt_Gallery_Page').click(function(){                                                   // return index WpDevArt_Gallery_Page
								localStorage.setItem('wpda_gall_gallery_active_tab_index','0');														
								wpda_gallery_edit_upload.restart_tree();
		});
	...
	select_with_shift_media:function(){	
		...
		chkboxes.click(function(event) {                                                        //01-04-2018
			event.stopPropagation();
			if(typeof(lastChecked)==='undefined') {
				lastChecked = this;
				return;
			}
			if(self.shift_key_preset || event.shiftKey) {                                        //01-04-2018
	...
	media_attachemnt_thumbnail_click:function(){
		var self=this;
		jQuery('#from_media_library .thumbnails.with_checkbox').click(function(event){	           //01-04-2018
			if(event.shiftKey){                                                                    //01-04-2018
		...
		jQuery('#from_media_library .thumbnails.with_checkbox input').click(function(event){        //01-04-2018
	...
	album_thumbnail_click:function(){
		var self=this;
		jQuery('#wpda_gall_edit_album_form .thumbnails.with_checkbox.with_edit').click(function(event){	                //28-03-2018
			if(event.shiftKey){
				self.shift_key_preset=true;
			}else{
				self.shift_key_preset=false;
		...
		jQuery('#wpda_gall_edit_album_form .thumbnails.with_checkbox.with_edit input').click(function(event){                //28-03-2018
			if(jQuery(this).is( ":checked" )){
				jQuery(this).parent().addClass('active')				
			}else{
				jQuery(this).parent().removeClass('active')
	...
	select_with_shift_album:function(){	
		...
		chkboxes.click(function(event) {                                 //28-03-2018
			event.stopPropagation();
			if(typeof(lastChecked)==='undefined') {
				lastChecked = this;
				return;
			}
			if(self.shift_key_preset || event.shiftKey) {                 //28-03-2018
				var start = chkboxes.index(this);
				var end = chkboxes.index(lastChecked);
				chkboxes.slice(Math.min(start,end), Math.max(start,end)+ 1).prop('checked', lastChecked.checked);
				if(lastChecked.checked){
	...
	album_thumbnail_edit_click:function(){
		...
		jQuery('#wpda_gall_edit_album_form .thumbnails.with_checkbox.with_edit span, #wpda_gall_edit_album_form table tr td span').click(function(event){ //28-03-2018
			var loc_self=this;
			event.stopPropagation();
	...
	create_pageintion_html:function(){
		...
		self.pagination_buttons[11].onclick = function() { 
			...
			//localStorage.setItem('wpda_gall_current_page_index',self.current_page_index);
			jQuery('#from_media_library .thumbnails.with_checkbox').remove();
			jQuery('#wpda_gall_edit_album_form .thumbnails.with_checkbox.with_edit').remove();
		...
		jQuery("#wpda_gall_load_table .left").click(function(){
			if(wpda_gall_load.open_close == 1){
				...
				jQuery.ajax({
					...
					success: function(data){//alert('left');
						jQuery('#wpda_gall_load_table #' + id + ' .load').removeClass('wpda_gall_display_inline');
						wpda_gall_load.k++;
						jQuery("#wpda_gall_load_table #" + id).children().eq(0).children().eq(0).css("background-image", "url('"+wpda_gall_content_url_width + data+"?" + wpda_gall_load.k + "')");
						jQuery("#wpda_gall_load_table #" + id).children().eq(4).children().eq(2).css("visibility","visible");
...
function wpda_gall_load_images() {	
	if (wpda_gall_load.countImages > 0 && wpda_gall_load.cancel == 1) {
		...
		jQuery.ajax({
			...
			success     : function(data){//alert('upload');
					var vv = 'li'+wpda_gall_load.array_images[wpda_gall_load.key];
					var im = 'im'+wpda_gall_load.array_images[wpda_gall_load.key];
